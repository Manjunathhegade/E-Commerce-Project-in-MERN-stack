...
class VodkaCategory extends React.Component {
constructor(props) {
    super(props);
...
componentDidMount() {
    ...
    this.VodkaCategory = "Vodka"
    axios.get(`http://localhost:5000/product/${this.VodkaCategory}`)
        .then(result => this.setState({ vodkaProducts: result.data }))
        .catch(error => console.log(error))
    console.log(this.state.vodkaProducts)
}
onView(Id) {
...
onSendCart(id,name,img,price){
    ...
    render() {
        ...
        return (
            ...
            <CategoryNav />
            <hr></hr>
            <h3>Category / Vodka </h3>
            <hr></hr>
            <div className="container">
                <div className="row" style={{margin:30}}>
                    {this.state.vodkaProducts.map((row) => (
                        <div className="card">
                        <br></br>
                        <div className="p-1">
